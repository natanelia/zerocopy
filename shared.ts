/** Worker transport for immutable snapshots. Wire version 2 is not compatible with v1 pointers. */
...
export async function initWorker<T extends Record<string, SharedStructure>>(data: WorkerData): Promise<T> {
  if (!data?.__shared || data.version !== FORMAT_VERSION) throw new Error('Unsupported worker data; create a v2 payload with getWorkerData()');
