...
const engineFiles = readdirSync(root).filter(p => p === 'persistent-core.as.ts' || p === 'arena.ts' || p === 'codec.ts' || p === 'types.ts' || p === 'wasm-utils.ts' || p === 'set-key.ts' || p === 'shared.ts' || /^shared-.*\.ts$/.test(p) && !p.endsWith('.test.ts') && !p.endsWith('.as.ts')).sort();
