...
for (const points of [16, 512, 16384, 131072]) {
  ...
  const simd = new WebAssembly.Instance(modules.simd, { env: { memory } }).exports;
  for (const extent of [1000, 6000]) {
    ...
    const fns = {
      ...
      'scalar-kernel': n => { let total = 0; for (let i = 0; i < n; i++) total += scalar.countPointsInBox(list.root, list.depth, list.tail, list.size, box.minX, box.minY, box.maxX, box.maxY); return total; },
      'simd-kernel': n => { let total = 0; for (let i = 0; i < n; i++) total += simd.countPointsInBox(list.root, list.depth, list.tail, list.size, box.minX, box.minY, box.maxX, box.maxY); return total; },
    ...
    const row = { points, extent, iterations, ms, samples, simdOverScalar: ms['scalar-kernel'] / ms['simd-kernel'], publicOverExisting: ms['existing-forEach'] / ms['public-auto'] };
    results.push(row); console.log('SPATIAL', JSON.stringify({ ...row, samples: undefined }));
  }
}
mkdirSync('proofs/results', { recursive: true });
writeFileSync(`proofs/results/spatial-${process.versions.bun ? 'bun' : 'node'}-${process.arch}.json`, JSON.stringify({ commit: process.env.GITHUB_SHA, run: process.env.GITHUB_RUN_ID, runtime: process.version, bun: process.versions.bun, arch: process.arch, cpu: cpus()[0].model, sink, results }, null, 2));
if (process.env.PERF_GATE === '1') for (const row of results.filter(r => r.points >= 512)) {
  assert.ok(row.simdOverScalar > 1.05, `SIMD did not beat scalar at ${row.points}/${row.extent}`);
  assert.ok(row.publicOverExisting > 1.05, `Public API did not beat existing at ${row.points}/${row.extent}`);
}
